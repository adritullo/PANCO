...
def PansharpenHPM(hiri_input, cas_input, outputName = "outputHPM.tif", band1 = 1, band2 = 2, band3 = 3):
    ...
    highpass_image = np.subtract(rasterArrayHiri, lowPassArrayHiRI, dtype=np.float64)
    ratio_r = np.divide(rasterArray1, lowPassArrayHiRI, dtype=np.float64)
    ratio_r[np.isinf(ratio_r)] = 0
    ratio_r[np.isnan(ratio_r)] = 0
    ratio_g = np.divide(rasterArray2, lowPassArrayHiRI, dtype=np.float64)
    ratio_g[np.isinf(ratio_g)] = 0
    ratio_g[np.isnan(ratio_g)] = 0
    ratio_b = np.divide(rasterArray3, lowPassArrayHiRI, dtype=np.float64)
    ratio_b[np.isinf(ratio_b)] = 0
    ratio_b[np.isnan(ratio_b)] = 0
...
if __name__ == '__main__':
    ...
    PansharpenHPM(PAN_input, MS_input, outputname, band1, band2, band3)
